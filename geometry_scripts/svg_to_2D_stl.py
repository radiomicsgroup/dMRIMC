...
# Import svg, convert to mesh, extrude, export, delete
for i in files:
    name = i
    # SVG
    bpy.ops.import_curve.svg(filepath=f"{folder}/{i}")
    # Select object
    ...
    # Get dimensions of the shape and use that to determine the size of the
    # z axis. Let's go for 1/3 of the smallest dim
    x, y, z = bpy.data.objects[0].dimensions
    # In Blender it says meters but we consider them mm. So since we
    # want 150μm we make it
    z_new = 0.150
    ...
    # new addition to simplify the mesh
    ...
    bpy.ops.export_mesh.stl(filepath=f"{stl_folder}/{name}.stl",
                            check_existing=True,
                            ascii=True,
                            use_selection=True)
